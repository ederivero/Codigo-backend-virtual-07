...
on alumnos_cursos.cursos_id = cursos.id; -- CUANDO alumnos_cursos.cursos_id sea igual cursos.id

-- 1. Los cursos con su cantidad de alumnos
select nombre, count(nombre) as 'cantidad de alumnos'
from cursos as c
...
select concat_ws(' ',matricula, nombre, apellido) as 'ALUMNO', count(*) 'CURSOS MATRICULADOS'
from alumnos as a
inner join alumnos_cursos as ac
...
having count(*) = (
    ...
    order by 1 desc
    limit 1
) -- el having se usa para filtro pero con funciones de agregacion, el having tbn se puede usar como un where
order by count(*) desc;
